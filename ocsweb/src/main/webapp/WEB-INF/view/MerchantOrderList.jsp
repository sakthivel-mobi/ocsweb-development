...
<!-- CDN moved to local -->
<script
	src="${pageContext.request.contextPath}/resources/js/signaturePad.js"
...
<body>
	<main class="container mt110" id="desktop-view">
						...
						<table id="table" class="table">
							<thead>
								<tr class="bcf4f4f4 c617075 fw500">
									...
									<th>Date</th>
									<th>Company Name</th>
									<th>Welcome Letter Acceptance</th>
								</tr>
							</thead>
							<tbody class="c3f3f3f fw500">
								<c:forEach var="item" items="${orders}">
									...
									<tr>
										<td>${item.id}</td>
										<td>${item.quotation.orderType}</td>
										<td>${item.createdOn}</td>
										<td>${item.quotation.companyName}</td>
										<td><a
											href="${pageContext.request.contextPath}/WelcomeLetter?quotationId=${item.quotation.id}"><button
													class="btn btn-mobi btn-sm" style="width: 100%">View
													Welcome Letter</button></a> <!-- <c:if test="${item.quotation.welcomeLetterAccepted != '1'}">
								
												</c:if>
												<c:if test="${item.quotation.welcomeLetterAccepted == '1'}">
													<button class="btn btn-secondary btn-sm" style="width: 100%"
														disabled="true">
														Welcome Letter Accepted </button>
												</c:if> --></td>
									</tr>
								</c:forEach>
							</tbody>
		...
		<div id="overlay"
			style="display: none; pointer-events: none; cursor: default; text-decoration: none;">
		</div>
		<div class="product-container" id="product-container"
			style="visibility: collapse;">
			<div class="product-header">
				<div class="d-flex flex-row">
					<h5 class="flex-grow-1" style="color: #535353"
						id="product-order-id"></h5>
					<button type="button" class="btn-close" aria-label="Close"
						onclick="onCloseProductContainer()"></button>
				</div>
			</div>
			<div class="product-body row mt-3 flex-grow-1"
				id="product-container-row"></div>
			<div class="product-footer">
				<div id="quotation-tearms-conditions"
					class="d-flex flex-row align-self-center">
					<div class="form-check">
						<input class="form-check-input" type="checkbox" value=""
							id="checkbox-accept-terms-conditions"> <label
							class="form-check-label" for="checkbox-accept-terms-conditions">
							I hereby agree and accept the <a
							href="https://gomobi.io/merchant-tnc" target="_blank">Terms
								and Conditions</a> and Quotations.
						</label>
					</div>
				</div>
				<hr />
				<div class="d-flex flex-row align-self-center">
					...
					<button class="btn btn-outline-primary btn-sm m-2"
						id="button-download-quotation">Download Quotation</button>
					<button class="btn btn-primary btn-sm m-2"
						id="button-accept-quotation">Accept Quotation</button>
				</div>
			</div>
		</div>
		...
		<!-- Modal -->
		<div class="modal fade" id="quotationAcceptanceModal" tabindex="-1"
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<div class="mt-4">
							<label for="namePerIc">Enter your Name as Per IC*</label> <input
								type="text" name="" value="" id="namePerIc" class="form-control">
						</div>

						<div class="mt-4">
							<label for="icNumber">Enter the IC NO*</label> <input type="text"
								name="" value="" id="icNumber" class="form-control">
						</div>
					</div>
					<div class="modal-footer">
						...
						<button type="button" class="btn btn-primary"
							onclick="acceptQuotation()">Accept Quotation</button>
					</div>
				</div>
			</div>
		</div>
		...
		<!-- Modal -->
		<div class="modal fade" id="newQuotationAcceptModal" tabindex="-1"
			aria-labelledby="exampleModalLabel" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
					...
					<div class="modal-body">
						...
						<div
							...
							<canvas id="signature-canvas" width="300" height="300"
								style="width: 300px !important; height: 300px !important; touch-action: none; margin: 1rem; border: 1px solid #0000002e; border-radius: 4px; padding: 8px;"></canvas>
						</div>

						<div class="mt-4">
							<label for="namePerIc-new">Enter your Name as Per IC*</label> <input
								type="text" name="" value="" id="namePerIc-new"
								class="form-control">
						</div>

						<div class="mt-4">
							<label for="icNumber-new">Enter the IC NO*</label> <input
								type="text" name="" value="" id="icNumber-new"
								class="form-control">
						</div>
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-secondary"
							data-bs-dismiss="modal">Close</button>
						<button type="button" class="btn btn-primary"
							onclick="acceptNewQuotation()">Accept Quotation</button>
					</div>
				</div>
			</div>
		</div>
	...
	<script>
			...
			checkBoxTermsAndConditions.addEventListener('change', () => {
				if (event.target.checked) {
					document.getElementById("button-accept-orderItem").disabled = false
				} else {
			...
			function getMerchantQuotationList() {
				...
				var config = {
					method: 'post',
					url: contextPath + "/merchant/order",
					headers: {
						'X-CSRF-TOKEN': token,
			...
			function loadOrderList(data) {
				...
				if (data.length > 0) {
					data.forEach(item => {
						...
						createdAtValue.innerHTML = item.createdAt
						...
						if (item.enableWelcomeLetter === false) {
							buttonViewOrder.href = contextPath + "/WelcomeLetter?quotationId=" + item.quotationId
							buttonViewOrder.innerText = "View Welcome Letter"
						} else {
							buttonViewOrder.setAttribute('readonly', true);
							buttonViewOrder.innerText = "Welcome Letter Accepted"
						}
			...
			function getQuotationId(quotationId) {
				...
				var config = {
					method: 'post',
					url: contextPath + "/merchant/orderItem/" + quotationId,
					headers: {
						'X-CSRF-TOKEN': token,
						'Content-Type': 'application/json'
					}
			...
			function showOrderLines(orderLine) {
				...
				orderLine.forEach(item => {
					...
					totalPrice = parseFloat(totalPrice) + parseFloat(item.product.unitPrice)
					...
					totalvalue.innerText = "RM " + parseFloat(item.product.unitPrice).toFixed(2) // Order
				...
				document.getElementById("totalPrice").innerText = "RM " + parseFloat(totalPrice).toFixed(2)
			}

			document.getElementById("button-download-orderItem").onclick = function () {
			...
			function downloadQuotation(quotationID) {
				const config = {
					method: 'post',
					url: contextPath + "/orderItem/download/" + quotationID,
					headers: {
						'X-CSRF-TOKEN': token,
						'Content-Type': 'application/json'
					}
			...
			document.getElementById("button-accept-orderItem").onclick = function () {
				productContainer.style.visibility = "collapse"
				overlay.style.display = "none"
			...
			function acceptQuotationWithSignature(selectedQuotationId, namePerIc, icNumber) {
				const config = {
					method: 'post',
					url: contextPath + '/merchant/orderItem/' + selectedQuotationId + '/accept',
					headers: {
						'X-CSRF-TOKEN': token,
						'Content-Type': 'application/json'
					},
