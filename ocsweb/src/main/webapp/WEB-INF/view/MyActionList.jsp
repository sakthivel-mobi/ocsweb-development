...
<html lang="en">
  <head>
	...
	<link rel="apple-touch-icon" sizes="60x60" href="${pageContext.request.contextPath}/resourcess/images/favicon/apple-icon-60x60.png">
	...
	<div class="row">
		<div class="col-md-12 col-lg-12">
			<div class="bg-white p-5 rounded">
				...
				<div class="row">
					<div class="col-md-6 col-lg-8 col-xs-12 c5d5d5d">My Actions</div>
...
<c:param name="id" value="${tempOrder.id}"></c:param>
 ...
 <c:choose>
    <c:when test="${tempOrder.quotation.orderLines[1].product.productType == 'EZYSPLIT'}">
        <c:set var = "price" scope = "session" value = "${tempOrder.quotation.orderLines[0].quotationEzysplitMDRRate.unitPrice}"/>
    </c:when>    
    <c:otherwise>
       <c:set var = "price" scope = "session" value = "${tempOrder.quotation.orderLines[0].quotationMDRRate.unitPrice}"/>
    </c:otherwise>
</c:choose>
				...
				<td><a href="${viewLink} ">${tempOrder.businessName}</a></td>
				<td>${tempOrder.quotation.salesPerson.aliasName}</td>
				<td>${tempOrder.quotation.contact.phoneNumber}</td>
				<td>${tempOrder.quotation.orderLines[0].product.productType}</td>
				<td>${tempOrder.stage}</td>
				<%-- <td><fmt:formatNumber type="number" maxFractionDigits="2" minFractionDigits="2" value="${price}"/></td> --%>
				<td><a  href="OrderView?id=${tempOrder.id}" class="btn btn-mobi btn-sm">View</a></td>
				</tr>
