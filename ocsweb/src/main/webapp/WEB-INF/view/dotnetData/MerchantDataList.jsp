			...
			<div class="col-md-12 col-lg-12">
				<div class="bg-white p-5 rounded">
					<form action="addQuotation" method="post">
						<div class="row">
							<div class="col-md-6 col-lg-8 col-xs-12 c5d5d5d">OCS Data
								v1</div>
					...
					<div class="table-responsive">
						<table id="table" class="table">
							...
							<tbody class="c3f3f3f fw500">
								<c:forEach var="tempMerchantDetail" items="${merchantDetails}">
									<c:url var="viewLink" value="/MerchantData/OrderView">
										<c:param name="id" value="${tempMerchantDetail.orderId}"></c:param>
									</c:url>

									<%-- <c:if test="${tempMerchantDetail.quotation.acquirer =='Paydee'}">
												<c:url var="viewLink" value="/MerchantData/OrderViewPaydee">
													<c:param name="id" value="${tempMerchantDetail.id}"></c:param>
												</c:url>


												</c:if>

												<c:if test="${tempMerchantDetail.quotation.acquirer =='U-Mobile'}">
													<c:url var="viewLink" value="/MerchantData/OrderView">
														<c:param name="id" value="${tempMerchantDetail.id}"></c:param>
													</c:url>
												</c:if> --%>
									<tr>
										<td>${tempMerchantDetail.orderId}</td>
										<%-- <td>${tempMerchantDetail.createdOn}</td> --%>
										<fmt:parseDate value="${tempMerchantDetail.createdOn}"
											pattern="yyyy-MM-dd HH:mm:ss" var="myDate" />
										<td><fmt:formatDate pattern="dd/MM/yyyy HH:mm:ss"
												value="${myDate}" /></td>
										<td><a href="${viewLink} ">${tempMerchantDetail.registeredName}</a>
										</td>
										<td>${tempMerchantDetail.userId}</td>
